...
export default function LLMSettingsPage(){
  ...
  return <PageContainer><Card><h2>Настройки / LLM настройки</h2><div className='goal-grid'>
    ...
    <Input value={f.model||''} onChange={e=>setF({...f,model:e.target.value})} placeholder='openrouter/free' />
    <Input value={f.api_key||''} onChange={e=>setF({...f,api_key:e.target.value})} placeholder='API Key (********4078)' />
    <Input type='number' value={f.max_tokens||256} onChange={e=>setF({...f,max_tokens:Number(e.target.value)})} placeholder='Max tokens' />
  ...
  </div><div style={{marginTop:12,display:'flex',gap:8,flexWrap:'wrap'}}><Button variant='primary' onClick={save}>Сохранить настройки</Button><Button onClick={test}>Проверить подключение</Button><Button variant='soft' onClick={()=>setF({...f,provider:'mock'})}>Вернуться к Mock</Button></div>
