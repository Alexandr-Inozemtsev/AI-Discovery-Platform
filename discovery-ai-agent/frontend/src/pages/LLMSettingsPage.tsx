...
export default function LLMSettingsPage(){
  const [f,setF]=useState<any>({provider:'mock',base_url:'https://openrouter.ai/api/v1',api_key:'',model:'deepseek/deepseek-chat-v3-0324:free',timeout_seconds:120,temperature:0.2}); const [msg,setMsg]=useState('')
  useEffect(()=>{api<any>('/settings/llm').then(setF)},[])
  const save=async()=>{try{await api('/settings/llm',{method:'PUT',body:JSON.stringify(f)}); setMsg('Настройки LLM сохранены'); window.dispatchEvent(new Event('llm-updated'))}catch(e:any){setMsg('Ошибка сохранения')}}
  const test=async()=>{try{const r=await api<any>('/settings/llm/test',{method:'POST',body:JSON.stringify(f)}); setMsg('Успешно: '+r.message)}catch(e:any){setMsg('Ошибка: '+e.message)}}
  ...
  </div><div style={{marginTop:12,display:'flex',gap:8}}><button className='btn primary' onClick={save}>Сохранить настройки</button><button className='btn' onClick={test}>Проверить подключение</button><button className='btn' onClick={()=>setF({...f,provider:'mock',base_url:'https://openrouter.ai/api/v1',model:'deepseek/deepseek-chat-v3-0324:free'})}>Вернуться к Mock</button></div>{msg && <p>{msg}</p>}</div>
}
