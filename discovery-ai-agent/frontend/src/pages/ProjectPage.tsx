  ...
  return <div className='workspace-single'>
    <section>
      ...
      <div className='ui-card'>
        ...
        <ErrorBoundary fallbackTitle='Ошибка рендера этапа'>{active==='CONTEXT' ? <ContextStage contextStatus={`Контекст: ${thinking?'обновляется...':structured?.indexing_status==='completed'?'загружен · знания извлечены':structured?.indexing_status==='requires_update'?'требует обновления':'не загружен'} · ${documents.length} документов · ${links.length} ссылок`} onGoProblem={()=>setActive('PROBLEM')} sourceTrace={(structured?.source_trace || knowledge?.source_trace || []) as any[]} contextInput={contextInput} onUpdateContextField={(key:string,value:string)=>setContextInput((prev:any)=>({...prev,[key]:value}))} documents={Array.isArray(documents)?documents:[]} links={(Array.isArray(links))?links:[]} knowledge={knowledge} runContextAnalyze={runContextAnalyze} onUpload={(e:any)=>{const files=Array.from(e.target.files||[]).map((f:any,idx:number)=>({id:`doc_${Date.now()}_${idx}`,name:f.name,type:f.type||'unknown',size:f.size,created_at:new Date().toISOString(),ai_status:'добавлен'})); setDocuments([...(documents||[]),...files]); setStructured((s:any)=>({...s,indexing_status:'requires_update'}))}} onDeleteDocument={removeDocument} onDeleteLink={removeLink} linkDraft={linkDraft} setLinkDraft={setLinkDraft} onAddLink={addLink} />: active==='PROBLEM' ? <div className='problem-workspace'>
