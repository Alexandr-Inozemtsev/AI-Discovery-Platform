...
export default function ProjectPage(){
  ...
  const removeDocument = async(index:number)=>{if(!confirm('Удалить файл из контекста?')) return; const next=(documents||[]).filter((_:any,i:number)=>i!==index); setDocuments(next); await saveContextInput()}
  const removeLink = async(index:number)=>{if(!confirm('Удалить ссылку из контекста?')) return; const next=(links||[]).filter((_:any,i:number)=>i!==index); setLinks(next); await saveContextInput()}
  const addLink = ()=>{const v=linkDraft.trim(); if(!v) return; setLinks([...(links||[]),{url:v,type:detectLinkType(v),status:'Добавлено',created_at:new Date().toISOString()} as any]); setLinkDraft('')}
  ...
  return <div className='workspace-single'>
    <section>
      ...
      <div className='ui-card'>
        ...
        <ErrorBoundary fallbackTitle='Ошибка рендера этапа'>{active==='CONTEXT' ? <ContextStage contextInput={contextInput} onUpdateContextField={(key:string,value:string)=>setContextInput((prev:any)=>({...prev,[key]:value}))} documents={Array.isArray(documents)&&documents.length?documents:demoDocs} links={(Array.isArray(links)&&links.length)?links:demoLinks} knowledge={knowledge} runContextAnalyze={runContextAnalyze} onUpload={(e:any)=>{const files=Array.from(e.target.files||[]).map((f:any)=>({name:f.name,type:f.type||'unknown',size:f.size,created_at:new Date().toISOString(),ai_status:'Проиндексирован'})); setDocuments([...(documents||[]),...files])}} onDeleteDocument={removeDocument} onDeleteLink={removeLink} linkDraft={linkDraft} setLinkDraft={setLinkDraft} onAddLink={addLink} />: active==='PROBLEM' ? <div className='problem-workspace'>
