...
export default function ProjectPage(){
  ...
  const answerQuestion=async(q:any)=>{const ans=(questionAnswers[q.id]||'').trim(); if(!ans) return; try{setThinking(true); const r=await api<any>(`/projects/${projectId}/stage/${active}/ask`,{method:'POST',body:JSON.stringify({question_id:q.id,message:ans})}); setStagePatch(r.patch); await loadArtifact(active); setMsg('Ответ сохранен и учтен для генерации')}catch(e:any){setMsg(e?.message||'Ошибка ответа')}finally{setThinking(false)}}
    ...
    <section>
      ...
      <div className='card'>
        ...
        <ErrorBoundary fallbackTitle='Ошибка рендера этапа'>{active==='CONTEXT' ? <div className='context-workspace refined'>
          ...
          <div className='problem-center card'><h4>Формулировка проблемы</h4><label className='sub'>Ручная формулировка от PO</label><textarea className='textarea' placeholder='В чём проблема?' value={problemDraft.main_problem||''} onChange={e=>setProblemDraft({...problemDraft,main_problem:e.target.value})}/><label className='sub'>Проблемы для БТ (AI, строгая нумерация)</label><textarea className='textarea' value={problemDraft.problem_statement||''} onChange={e=>setProblemDraft({...problemDraft,problem_statement:e.target.value})}/><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button className='btn primary' onClick={generateProblem} disabled={thinking}>{thinking?'Генерация...':'Сгенерировать'}</button><button className='btn' onClick={generateProblem}>Перегенерировать</button><button className='btn' onClick={()=>saveProblem()}>Сохранить</button><button className='btn' onClick={()=>saveProblem('ready')}>Принять как финальную проблему</button><button className='btn' onClick={()=>{const v=(problemDraft.versions||[]).slice(-1)[0]?.snapshot; if(v) setProblemDraft({...problemDraft,...v})}}>Вернуть предыдущую версию</button></div><div className='sub'>Статус: {safeText(problemDraft.status||'draft')}</div></div>
          <div className='problem-right card'><h4>AI уточняющие вопросы</h4><button className='btn' onClick={regenerateStageQuestions} disabled={thinking}>{thinking?'Генерация...':'Сгенерировать дополнительные вопросы'}</button><div className='ai-sections'>{(problemDraft.clarifying_questions||[]).map((q:any,i:number)=>{const obj=typeof q==='string'?{id:`q_${i}`,text:q,answer:''}:q; return <div key={obj.id||i} className='card'><b>Вопрос {i+1}</b><div className='sub'>{safeText(obj.text||obj)}</div><input className='input' placeholder='Ваш ответ...' value={questionAnswers[obj.id] ?? obj.answer ?? ''} onChange={e=>setQuestionAnswers({...questionAnswers,[obj.id]:e.target.value})}/><button className='btn' onClick={()=>answerQuestion(obj)}>Сохранить ответ</button></div>})}{(problemDraft.ai_chat_history||[]).map((m:any,i:number)=><div key={i} className='card'><b>{m.role==='user'?'Вы':'AI'}</b><div className='sub'>{safeText(m.text)}</div></div>)}</div>{stagePatch && <div className='card'><div className='sub'>AI предлагает изменения.</div><div style={{display:'flex',gap:8}}><button className='btn primary' onClick={applyStagePatch}>Применить</button><button className='btn' onClick={()=>setStagePatch(null)}>Отклонить</button></div></div>}</div>
