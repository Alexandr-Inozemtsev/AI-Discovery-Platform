...
type GoalDraft = {
  ...
  successMetrics:GoalMetric[]; nonGoals:string[]; assumptions:string[]; openQuestions?:string[]; risks:string[]; constraints:string[]; stakeholders:string[];
...
export default function ProjectPage(){
  ...
  const applyAssistantPatch=async()=>{
    if(!projectId || !assistantSessionId || !assistantPendingAction){
      setAssistantError('Нет подготовленных изменений для применения.')
      return
    }
    try{
      ...
      setMsg('Изменения применены в артефакт после предпросмотра')
    }catch(e:any){
      setAssistantError(e?.message||'Ошибка применения изменений')
    }finally{
  ...
  const save=async()=>{try{setSaving('saving');const baseMeta={stage:active,status:'manually_edited',source_artifacts:stageOrder.slice(0,Math.max(0,stageOrder.indexOf(active))),source_versions:{},version:ver||0,updated_at:new Date().toISOString(),generated_at:null}; const payload=active==='GOAL'?{content:smartToText(goalData),structured_content:{...goalData,pipeline_meta:baseMeta},rich_content_json:richJson,rendered_html:content}:active==='CONTEXT'?{content:'',structured_content:{...buildContextPayload(),pipeline_meta:baseMeta},rich_content_json:null,rendered_html:null}:{content,structured_content:{...(structured||{}),pipeline_meta:baseMeta},rich_content_json:richJson,rendered_html:content}; const a=await api<any>(`/projects/${projectId}/artifacts/${active}`,{method:'PUT',body:JSON.stringify(payload)});setStructured(a.structured_content||structured);setVer(a.version);setUpdated(a.updated_at);setMsg('Сохранено');setSaving('saved'); await markDependentsStale(active,a.version); await loadCompletion()}catch{setMsg('Ошибка сохранения');setSaving('error')}}
  ...
  const normalizeGoalMetric=(metric:any):GoalMetric=>({metric:metric?.metric||metric?.name||'', currentValue:metric?.current_value||metric?.currentValue||'', targetValue:metric?.target_value||metric?.targetValue||metric?.target||'', measurement:metric?.measurement||'', dataSource:metric?.data_source||metric?.dataSource||metric?.source||''})
  const updateGoal=(patch:Partial<GoalDraft>)=>setStructured({...goalData,...patch,updatedAt:new Date().toISOString()})
  const goalData:GoalDraft = { ...emptyGoal(), ...(structured||{}), desiredOutcome: structured?.desired_outcome || structured?.desiredOutcome || '', successMetrics: (structured?.success_metrics || structured?.successMetrics || []).map(normalizeGoalMetric), nonGoals: structured?.non_goals || structured?.nonGoals || [], constraints: structured?.constraints || [], assumptions: structured?.assumptions || [], openQuestions: structured?.open_questions || structured?.openQuestions || [], smartAnalysis: structured?.smart_analysis || structured?.smartAnalysis || emptyGoal().smartAnalysis }
  ...
  const applyStagePatch=async()=>{if(!stagePatch) return; try{setThinking(true); const r=await api<any>(`/projects/${projectId}/stage/${active}/apply-patch`,{method:'POST',body:JSON.stringify({patch:stagePatch})}); setStructured(r.structured_content||structured); setStagePatch(null); setMsg('AI-изменения применены')}catch(e:any){setMsg(e?.message||'Ошибка применения изменений')}finally{setThinking(false)}}
  ...
  const renderList=(items:any[],empty:string)=>items.length?items.map((item:any,i:number)=><div className='stage-row' key={i}>{safeText(item)}</div>):<p className='sub'>{empty}</p>
  const renderEvidence=(items:any[])=>items.length?items.map((item:any,i:number)=><div className='stage-evidence' key={i}><b>{safeText(item.source_name||item.source_id||`Источник ${i+1}`)}</b><span>{safeText(item.chunk_id||item.content_level||'подтверждение')}</span></div>):<p className='sub'>Подтверждения не прикреплены.</p>
  const renderStructuredStage=()=>{
    if(active==='BUSINESS_EFFECT'){
      const qualitative=firstArray(structured.qualitative_effects, structured.qualitative_effect, structured.quality_effects)
      const quantitative=firstArray(structured.quantitative_metrics, structured.quantitative_effect, structured.metrics)
      const riskReduction=asArray(structured.risk_reduction)
      const operational=asArray(structured.operational_effect)
      const measurement=asArray(structured.measurement_method)
      const assumptions=asArray(structured.assumptions)
      const questions=asArray(structured.open_questions)
      return <div className='stage-specific-grid stage-specific-grid--wide'>
        <div className='ui-card'><h4>Качественный эффект</h4>{renderList(qualitative,'Пока нет подтверждённого качественного эффекта.')}</div>
        <div className='ui-card'><h4>Количественные метрики</h4>{quantitative.length?quantitative.map((item:any,i:number)=><div className='stage-row' key={i}><b>{safeText(item.metric||item.name||`Метрика ${i+1}`)}</b><span className='sub'>{safeText(item.value||item.target_value||item.target||item.direction||item.formula)}</span><span className={`ui-badge ${item.confidence==='confirmed'?'ready':'warning'}`}>{safeText(item.confidence||'assumption')}</span></div>):<p className='sub'>Если расчёт невозможен, AI должен предложить вопросы вместо неподтверждённых цифр.</p>}</div>
        <div className='ui-card'><h4>Снижение рисков</h4>{renderList(riskReduction,'Риски пока не описаны.')}</div>
        <div className='ui-card'><h4>Операционный эффект</h4>{renderList(operational,'Операционный эффект пока не описан.')}</div>
        <div className='ui-card'><h4>Метод измерения</h4>{renderList(measurement,'Метод измерения не указан.')}</div>
        <div className='ui-card'><h4>Допущения и вопросы</h4>{renderList([...assumptions,...questions],'Нет допущений и открытых вопросов.')}</div>
      </div>
    }
    if(active==='USE_CASES'){
      const useCases=firstArray(structured.use_cases, structured.uc_cards)
      return <div className='stage-specific-grid'>{useCases.length?useCases.map((uc:any,i:number)=><div className='ui-card uc-card' key={i}><div className='ui-actions ui-actions--between'><h4>{safeText(uc.id||`UC-${i+1}`)} · {safeText(uc.title||'Без названия')}</h4><span className='ui-badge warning'>{safeText(uc.actor||'Роль не указана')}</span></div><p className='sub'>Цель: {safeText(uc.goal||uc.description||'—')}</p><p className='sub'>Триггер: {safeText(uc.trigger||'—')}</p><h5>Предусловия</h5>{renderList(asArray(uc.preconditions),'Предусловия не указаны.')}<h5>Основной поток</h5>{asArray(uc.flow||uc.main_flow).map((step:any,idx:number)=><div className='stage-row' key={idx}>{idx+1}. {safeText(step)}</div>)}<h5>Альтернативные потоки</h5>{renderList(asArray(uc.alternative_flows),'Альтернативные потоки не указаны.')}<h5>Исключения</h5>{renderList(asArray(uc.exceptions),'Исключения не указаны.')}<h5>Постусловия</h5>{renderList(asArray(uc.postconditions),'Постусловия не указаны.')}<h5>Связанные требования</h5><div className='sub'>{asArray(uc.linked_requirements).length?safeText(uc.linked_requirements):'Нет связанного требования'}</div><h5>Подтверждения</h5>{renderEvidence(asArray(uc.evidence))}</div>):<div className='ui-card'><p className='sub'>Use Cases пока не сформированы. Используйте AI Chat для подготовки карточек UC.</p></div>}</div>
    }
    if(active==='FUNCTIONAL_REQUIREMENTS'){
      ...
      const warnings=asArray(structured.validation_warnings||structured.warnings)
      return <div className='stage-specific-grid stage-specific-grid--wide'>
        <div className='ui-card'><h4>FR/NFR таблица</h4><table className='ui-table'><thead><tr><th>ID</th><th>Тип</th><th>Формулировка</th><th>Связь</th><th>Критерии приёмки</th><th>Подтверждения</th><th>Статус</th></tr></thead><tbody>{[...fr.map((r:any)=>({...r,type:'FR'})),...nfr.map((r:any)=>({...r,type:'NFR'}))].map((r:any,i:number)=><tr key={i}><td>{safeText(r.id||`${r.type}-${i+1}`)}</td><td>{safeText(r.type)}</td><td>{safeText(r.text||r.title||r.description||r.target||r.target_value)}</td><td>{safeText(r.linked_use_case||r.category||'—')}</td><td>{safeText(r.acceptance_criteria||r.business_rules||r.measurement)}</td><td>{asArray(r.evidence).length?'Есть':'Нет'}</td><td><span className={`ui-badge ${r.assumption?'warning':'ready'}`}>{r.assumption?'Допущение':'Подтверждено'}</span></td></tr>)}</tbody></table>{!fr.length&&!nfr.length?<p className='sub'>Требования пока не сформированы.</p>:null}</div>
        <div className='ui-card'><h4>Статус проверки</h4>{warnings.length?warnings.map((w:any,i:number)=><div className='sub' key={i}>• {safeText(w)}</div>):<p className='sub'>Нет предупреждений в structured state.</p>}</div>
      </div>
    }
    if(active==='FINAL_BT'){
      const preview=structured.document_preview||{}
      const sections=firstArray(structured.sections, preview.sections)
      const validation=structured.validation_summary||{}
      const warnings=firstArray(structured.validation_warnings, validation.warnings)
      const unresolved=asArray(structured.unresolved_questions)
      const exportState=structured.export||{}
      return <div className='stage-specific-grid'>
        <div className='ui-card'><h4>Предпросмотр документа</h4><h3>{safeText(preview.title||'Финальный БТ')}</h3>{sections.length?sections.map((s:any,i:number)=><div className='stage-row' key={i}><b>{safeText(s.title||s.id||`Раздел ${i+1}`)}</b><span className='sub'>{safeText(s.content||s)}</span></div>):<p className='sub'>Разделы документа пока не собраны.</p>}</div>
        <div className='ui-card'><h4>Предупреждения проверки</h4>{warnings.length?warnings.map((w:any,i:number)=><div className='sub' key={i}>• {safeText(w)}</div>):<p className='sub'>Перед экспортом проверьте полноту Context, Problem, Goal и Requirements.</p>}</div>
        <div className='ui-card'><h4>Нерешённые вопросы</h4>{renderList(unresolved,'Нерешённые вопросы не указаны.')}</div>
        <div className='ui-card'><h4>Экспорт</h4><p className='sub'>Статус: {safeText(exportState.ready?'готов':'требует проверки')}</p><ButtonLink variant='secondary' size='sm' href={`http://localhost:8000/api/projects/${projectId}/export/docx`}>Экспортировать DOCX</ButtonLink></div>
      </div>
    }
    ...
    <section className='project-workspace-main'>
      <div className='ui-card'>
        ...
        <ErrorBoundary fallbackTitle='Ошибка рендера этапа'>{active==='CONTEXT' ? <ContextStage contextStatus={contextStatusLabel} onGoProblem={goProblemFromContext} sourceTrace={sourceTrace} contextInput={contextInput} onUpdateContextField={updateContextField} documents={Array.isArray(documents)?documents:[]} links={(Array.isArray(links))?links:[]} knowledge={knowledge} coverage={contextCoverage} readiness={contextReadiness} missingInformation={contextMissing} problemHandoff={problemHandoff} indexingStatus={structured?.indexing_status||'idle'} runContextAnalyze={runContextAnalyze} onUpload={uploadContextFiles} onDeleteDocument={removeDocument} onDeleteLink={removeLink} linkDraft={linkDraft} setLinkDraft={setLinkDraft} onAddLink={addLink} />: active==='PROBLEM' ? <div className='problem-workspace'>
          ...
          <div className='problem-center ui-card'><h4>Формулировка проблемы</h4><label className='sub'>Ручная формулировка от PO</label><textarea className='ui-textarea' placeholder='В чём проблема?' value={problemDraft.main_problem||''} onChange={e=>setProblemDraft({...problemDraft,main_problem:e.target.value})}/><label className='sub'>Формулировка для БТ</label><textarea className='ui-textarea' value={problemDraft.problem_statement||''} onChange={e=>setProblemDraft({...problemDraft,problem_statement:e.target.value})}/><div className='stage-two-cols'><div><label className='sub'>Пользовательские боли</label><textarea className='ui-textarea ui-textarea--md' value={asArray(problemDraft.user_pains).join('\n')} onChange={e=>setProblemDraft({...problemDraft,user_pains:e.target.value.split('\n').filter(Boolean)})}/></div><div><label className='sub'>Бизнес-боли</label><textarea className='ui-textarea ui-textarea--md' value={asArray(problemDraft.business_pains).join('\n')} onChange={e=>setProblemDraft({...problemDraft,business_pains:e.target.value.split('\n').filter(Boolean)})}/></div><div><label className='sub'>Корневые причины</label><textarea className='ui-textarea ui-textarea--md' value={asArray(problemDraft.root_causes).join('\n')} onChange={e=>setProblemDraft({...problemDraft,root_causes:e.target.value.split('\n').filter(Boolean)})}/></div><div><label className='sub'>Последствия, если не решить</label><textarea className='ui-textarea ui-textarea--md' value={asArray(problemDraft.consequences_if_not_solved).join('\n')} onChange={e=>setProblemDraft({...problemDraft,consequences_if_not_solved:e.target.value.split('\n').filter(Boolean)})}/></div></div><div className='stage-two-cols'><div><h5>Подтверждения</h5>{renderEvidence(asArray(problemDraft.evidence))}</div><div><h5>Допущения и вопросы</h5>{renderList([...asArray(problemDraft.assumptions),...asArray(problemDraft.open_questions)],'Допущения и вопросы не указаны.')}</div></div><div className='ui-card-footer'><Button variant='primary' size='md' onClick={generateProblem} disabled={thinking}>{thinking?'Генерация...':'Сгенерировать'}</Button><Button variant='secondary' size='md' onClick={()=>saveProblem()}>Сохранить</Button><Button variant='soft' size='md' onClick={()=>saveProblem('ready')}>Принять как финальную проблему</Button><Button variant='ghost' size='md' onClick={()=>{const v=(problemDraft.versions||[]).slice(-1)[0]?.snapshot; if(v) setProblemDraft({...problemDraft,...v})}}>Вернуть предыдущую версию</Button></div><div className='sub'>Статус: {safeText(problemDraft.status||'draft')}</div></div>
          ...
          <div className='ui-card'><h4>Целевая формулировка</h4>
            ...
            <textarea className='ui-textarea ui-textarea--md' placeholder='Что не входит в границы цели' value={(goalData.nonGoals||[]).join('\n')} onChange={e=>updateGoal({nonGoals:e.target.value.split('\n').filter(Boolean)})}/>
            ...
            <textarea className='ui-textarea ui-textarea--md' placeholder='Открытые вопросы' value={(goalData.openQuestions||[]).join('\n')} onChange={e=>updateGoal({openQuestions:e.target.value.split('\n').filter(Boolean)})}/>
          </div>
          ...
          <div className='ui-card'><h4>AI-черновики</h4>{goalDraft ? (goalDraft.goal_options||[]).map((o:any,i:number)=><div key={i} className='ui-card'><b>{o.title||`Вариант ${i+1}`}</b><div className='sub'>{o.description}</div><div className='ui-actions'><Button variant='primary' onClick={()=>applyGoalOption(o,'select')}>Выбрать</Button><Button variant='secondary' onClick={()=>applyGoalOption(o,'merge')}>Объединить</Button><Button variant='secondary'>Редактировать</Button></div></div>) : <div className='sub'>Сначала запустите генерацию целей.</div>}</div>
          <div className='ui-card'><h4>Карта влияния</h4><div className='impact-col'><span>Цель → Проблемы → KPI → Бизнес-эффект → Требования → Use Cases → Финальный БТ</span></div></div>
          <div className='ui-card'><h4>Вопросы AI</h4>{(goalData.aiQuestions||[]).map((q:string,i:number)=><div key={i} className='sub'>• {q}</div>)}<input className='ui-input' placeholder='Ответьте на вопрос AI...' value={goalQuestion} onChange={e=>setGoalQuestion(e.target.value)}/><Button variant='secondary' onClick={askGoal}>Отправить AI</Button>{goalPatch && <div className='ui-card'><div className='sub'>AI предлагает изменения</div><div className='ui-actions'><Button variant='primary' onClick={applyGoalPatch}>Применить</Button><Button variant='secondary' onClick={()=>setGoalPatch(null)}>Отклонить</Button></div></div>}</div>
