  ...
  return <div className='workspace-single'>
    <section>
      ...
      <div className='ui-card'>
        ...
        <ErrorBoundary fallbackTitle='Ошибка рендера этапа'>{active==='CONTEXT' ? <div className='context-layout'>
          ...
          <div className='context-coverage card'><h4>Покрытие знаний</h4><ul className='coverage-list'>{coverageRows.map(([label,status])=><li key={label} className='coverage-item'><span className='coverage-left'>{status==='success'?<CheckCircle2 size={16} className='ok'/>:status==='warning'?<AlertTriangle size={16} className='warn'/>:<XCircle size={16} className='bad'/>}<span>{label}</span></span><span className={`ui-badge ${status==='success'?'ready':status==='warning'?'warning':'error'}`}>{status==='success'?'OK':status==='warning'?'Нужно уточнить':'Нет данных'}</span></li>)}</ul><div className='hint-card'><b>Что ещё можно добавить</b><ul><li>Добавьте BPMN текущего процесса</li><li>Уточните KPI и целевые метрики</li><li>Добавьте ограничения и допущения</li></ul></div></div>
          <div className='ai-assistant ui-card'><h4>AI-ассистент</h4><div className='assistant-card'><b>Рекомендации</b><p className='sub'>Добавьте BPMN диаграмму AS IS для более точного анализа.</p><Button variant='soft' size='sm'>Загрузить BPMN</Button></div><div className='assistant-card'><b>Следующий шаг</b><p className='sub'>Перейдите к разделу «Проблема» для анализа.</p><Button variant='secondary' size='sm'>Перейти к проблеме</Button></div><div className='assistant-card'><b>История индексации</b><p className='sub'>Успешно: документы и ссылки проиндексированы · {new Date().toLocaleDateString('ru-RU')}</p><Button variant='secondary' size='sm'>Просмотреть историю</Button></div><div className='ui-actions'><input className='ui-input' placeholder='Спросите что угодно...'/><Button variant='icon' size='sm' aria-label='Отправить'><Send size={16}/></Button></div></div>
