    ...
    <section>
      ...
      <div className='card'>
        ...
        {active==='CONTEXT' ? <div className='context-workspace refined'>
          ...
          <div className='card'><h4>AI Goal Engine</h4>{goalDraft && <div className='card'><b>AI Generated Goal Drafts</b>{(goalDraft.goal_options||[]).map((o:any,i:number)=><div key={i} className='card'><b>{o.title||`Вариант ${i+1}`}</b><p className='sub'>{o.description}</p><div className='sub'>KPI: {(o.suggested_kpis||[]).join(', ')||'—'}</div><div style={{display:'flex',gap:8,marginTop:8}}><button className='btn primary' onClick={()=>applyGoalOption(o,'select')}>Выбрать</button><button className='btn' onClick={()=>applyGoalOption(o,'merge')}>Объединить</button><button className='btn' onClick={()=>setMsg('Отредактируйте поля цели вручную справа')}>Редактировать</button></div></div>)}<div className='card'><b>Рекомендованная цель</b><div className='sub'>{goalDraft?.recommended_goal?.title||'—'}</div><button className='btn primary' onClick={applyRecommendedGoal}>Применить рекомендованную цель</button></div></div>}<div className='ai-sections'><div className='card'><b>AI Summary</b><p className='sub'>{goalData.aiSummary||'После запуска AI-анализа здесь появится краткая сводка.'}</p></div><div className='card'><b>SMART-анализ</b>{Object.entries(goalData.smartAnalysis||{}).map(([k,v]:any)=><div key={k} className='sub'>{v.status==='ok'?'✅':v.status==='warning'?'⚠️':'❌'} {k}: {v.comment}. Рекомендация: {v.recommendation}</div>)}</div><div className='card'><b>AI Questions</b>{toArray(goalData.aiQuestions).map((q:string,i:number)=><div key={i} className='sub'>• {q}</div>)}</div><div className='card'><b>AI Contradictions</b>{toArray(goalData.aiContradictions).map((q:string,i:number)=><div key={i} className='sub'>• {q}</div>)}</div><div className='card'><b>Completeness: {goalData.completeness||0}%</b><div className='progress'><div style={{width:`${goalData.completeness||0}%`}}/></div></div></div></div>
