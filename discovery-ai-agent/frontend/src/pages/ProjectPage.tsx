  ...
  return <div className='workspace-single'>
    <section>
      ...
      <div className='card'>
        ...
        {active==='FINAL_BT' && <div className='card' style={{marginTop:12}}><h4 style={{marginTop:0}}>Preview финального БТ</h4><div className='sub'>Sections, зависящие от Goal: BUSINESS_EFFECT, AS_IS, TO_BE, USE_CASES, REQUIREMENTS, RISKS, FINAL_BT.</div></div>}<div className='card' style={{marginTop:12}}><h4 style={{marginTop:0}}>AI уточняющие вопросы</h4><input className='input' placeholder='Ответьте AI...' value={stageQuestion} onChange={e=>setStageQuestion(e.target.value)}/><button className='btn' onClick={askStageQuestion} disabled={thinking}>Отправить</button>{stagePatch && <div className='card'><div className='sub'>AI предлагает изменения в этот этап.</div><div style={{display:'flex',gap:8}}><button className='btn primary' onClick={applyStagePatch}>Применить</button><button className='btn' onClick={()=>setStagePatch(null)}>Отклонить</button></div></div>}</div><div className='sub'>Autosave: {saving==='saving'?'Сохранение…':saving==='saved'?'Сохранено':saving==='error'?'Ошибка сохранения':'—'}</div>
