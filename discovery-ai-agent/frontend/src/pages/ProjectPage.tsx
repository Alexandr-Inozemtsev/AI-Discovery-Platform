    ...
    <section>
      ...
      <div className='ui-card'>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'start',gap:8}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}><span className='sub'>Версия: {ver??0}</span><span className='sub'>Обновлено: {updated?new Date(updated).toLocaleString('ru-RU'):'—'}</span><Button variant='primary' size='sm' onClick={save}>Сохранить</Button><Button variant='secondary' size='sm' onClick={gen} disabled={busy}>Сгенерировать</Button><Button variant='secondary' size='sm' onClick={validate} disabled={busy}>Проверить</Button><ButtonLink variant='icon' size='sm' href={`http://localhost:8000/api/projects/${projectId}/export/docx`}><Download size={16}/></ButtonLink><Button variant='icon' size='sm' aria-label='Дополнительные действия'><MoreHorizontal size={16}/></Button></div>
        </div>
        ...
        <ErrorBoundary fallbackTitle='Ошибка рендера этапа'>{active==='CONTEXT' ? <div className='context-workspace refined'>
          ...
          <div className='context-center card'>
            ...
            <div className='sources-card'>
              <div className='sources-head'><b>Документы ({(Array.isArray(documents)&&documents.length)?documents.length:3})</b><label className='ui-btn ui-btn--secondary ui-btn--sm'>Загрузить файлы<input type='file' multiple style={{display:'none'}} onChange={e=>{const files=Array.from(e.target.files||[]).map((f:any)=>({name:f.name,type:f.type||'unknown',size:f.size,created_at:new Date().toISOString(),ai_status:'Проиндексирован'})); setDocuments([...(documents||[]),...files])}}/></label></div>
            ...
            <div className='sources-card'>
              ...
              <div className='sources-add'><input className='ui-input' placeholder='Добавить ссылку...' value={linkDraft} onChange={e=>setLinkDraft(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault(); const v=linkDraft.trim(); if(v){setLinks([...(links||[]),JSON.stringify({url:v,type:detectLinkType(v),status:'Добавлено, ожидает обработки',created_at:new Date().toISOString()})]); setLinkDraft('')}}}}/><Button variant='primary' onClick={()=>{const v=linkDraft.trim(); if(!v) return; setLinks([...(links||[]),JSON.stringify({url:v,type:detectLinkType(v),status:'Добавлено, ожидает обработки',created_at:new Date().toISOString()})]); setLinkDraft('')}}>Добавить</Button></div>
            </div>
          </div>
          <div className='context-right card'>
            <h4><Database size={16}/> Извлечённые знания (AI)</h4><div className='ui-actions'><Button variant='secondary' onClick={runContextAnalyze}>Обновить извлечение</Button><Button variant='secondary' onClick={()=>setMsg('Показаны все извлечённые знания')}>Показать все извлечённые знания</Button><Button variant='secondary' onClick={()=>setMsg('Скопировано в следующий этап')}>Скопировать в следующий этап</Button></div>
            ...
            <Button variant='secondary' onClick={runContextAnalyze}><RefreshCcw size={14}/> Переиндексировать</Button>
          </div>
          ...
          <div className='ai-assistant ui-card'><h4>AI-ассистент</h4><div className='ui-card'><b>Рекомендации</b><p className='sub'>Добавьте BPMN диаграмму AS IS для более точного анализа.</p></div><div className='ui-card'><b>Следующий шаг</b><p className='sub'>Перейдите к разделу «Проблема» для анализа.</p><Button variant='secondary'>Перейти к проблеме</Button></div><div className='ui-card'><b>История индексации</b><p className='sub'>Успешно: документы и ссылки проиндексированы.</p></div><div style={{marginTop:'auto'}}><input className='ui-input' placeholder='Спросите что угодно...'/></div></div>
