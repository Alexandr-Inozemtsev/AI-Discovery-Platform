    ...
    <section>
      ...
      <div className='ui-card'>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'start',gap:8}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}><span className='sub'>Версия: {ver??0}</span><span className='sub'>Обновлено: {updated?new Date(updated).toLocaleString('ru-RU'):'—'}</span><button className='ui-btn primary' onClick={save}>Сохранить</button><button className='ui-btn secondary' onClick={gen} disabled={busy}>Сгенерировать</button><button className='ui-btn secondary' onClick={validate} disabled={busy}>Проверить</button><a className='ui-btn secondary' href={`http://localhost:8000/api/projects/${projectId}/export/docx`}><Download size={16}/></a><button className='ui-btn secondary'><MoreHorizontal size={16}/></button></div>
        </div>
        ...
        <ErrorBoundary fallbackTitle='Ошибка рендера этапа'>{active==='CONTEXT' ? <div className='context-workspace refined'>
          <div className='context-left card'>
            <h4>Обзор проекта</h4>
            <input className='ui-input' placeholder='Название инициативы' value={contextInput.initiative_name} onChange={e=>setContextInput({...contextInput,initiative_name:e.target.value})}/>
            <textarea className='ui-textarea' placeholder='Краткое описание' value={contextInput.short_description} onChange={e=>setContextInput({...contextInput,short_description:e.target.value})}/>
            <input className='ui-input' placeholder='Цель инициативы' value={contextInput.initiative_goal} onChange={e=>setContextInput({...contextInput,initiative_goal:e.target.value})}/>
            <input className='ui-input' placeholder='Бизнес-направление' value={contextInput.business_domain} onChange={e=>setContextInput({...contextInput,business_domain:e.target.value})}/>
            <input className='ui-input' placeholder='Владелец процесса' value={contextInput.process_owner} onChange={e=>setContextInput({...contextInput,process_owner:e.target.value})}/>
            <input className='ui-input' placeholder='Владелец Discovery' value={contextInput.discovery_owner} onChange={e=>setContextInput({...contextInput,discovery_owner:e.target.value})}/>
            <input className='ui-input' placeholder='Связанные процессы' value={contextInput.related_processes} onChange={e=>setContextInput({...contextInput,related_processes:e.target.value})}/>
            <input className='ui-input' placeholder='Связанные системы' value={contextInput.related_systems} onChange={e=>setContextInput({...contextInput,related_systems:e.target.value})}/>
          </div>
          <div className='context-center card'>
            ...
            <div className='sources-card'>
              <div className='sources-head'><b>Документы ({(Array.isArray(documents)&&documents.length)?documents.length:3})</b><label className='ui-btn secondary'>Загрузить файлы<input type='file' multiple style={{display:'none'}} onChange={e=>{const files=Array.from(e.target.files||[]).map((f:any)=>({name:f.name,type:f.type||'unknown',size:f.size,created_at:new Date().toISOString(),ai_status:'Проиндексирован'})); setDocuments([...(documents||[]),...files])}}/></label></div>
            ...
            <div className='sources-card'>
              ...
              <div className='sources-add'><input className='ui-input' placeholder='Добавить ссылку...' value={linkDraft} onChange={e=>setLinkDraft(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault(); const v=linkDraft.trim(); if(v){setLinks([...(links||[]),JSON.stringify({url:v,type:detectLinkType(v),status:'Добавлено, ожидает обработки',created_at:new Date().toISOString()})]); setLinkDraft('')}}}}/><button className='ui-btn primary' onClick={()=>{const v=linkDraft.trim(); if(!v) return; setLinks([...(links||[]),JSON.stringify({url:v,type:detectLinkType(v),status:'Добавлено, ожидает обработки',created_at:new Date().toISOString()})]); setLinkDraft('')}}>Добавить</button></div>
            </div>
          </div>
          <div className='context-right card'>
            <h4><Database size={16}/> Извлечённые знания (AI)</h4><div style={{display:'flex',gap:8,marginBottom:8}}><button className='ui-btn secondary' onClick={runContextAnalyze}>Обновить извлечение</button><button className='ui-btn secondary' onClick={()=>setMsg('Показаны все извлечённые знания')}>Показать все извлечённые знания</button><button className='ui-btn secondary' onClick={()=>setMsg('Скопировано в следующий этап')}>Скопировать в следующий этап</button></div>
            {knowledge ? <div className='ai-sections'>{['процессы','системы','роли','интеграции','kpi','бизнес_сущности','документы','термины'].map((k)=><div className='ui-card' key={k}><b>{k}</b><div className='timeline'>{Array.isArray(knowledge[k]) ? knowledge[k].map((x:any,i:number)=><span key={i} className='ui-badge progress'>{safeText(x)}</span>) : []}</div></div>)}<div className='ui-card'><b>Покрытие знаний</b><ul>{Object.entries(knowledge?.покрытие||knowledge?.coverage||{}).map(([k,v]:any)=><li key={k}>{safeText(v)} — {k}</li>)}</ul><div className='sub'>Что ещё можно добавить: BPMN, KPI, SLA, ограничения, Jira/Confluence/Figma ссылки.</div></div></div> : <p className='sub'>После индексации здесь появятся извлечённые знания.</p>}
            <button className='ui-btn secondary' onClick={runContextAnalyze}><RefreshCcw size={14}/> Переиндексировать</button>
          </div>
          ...
          <div className='ai-assistant ui-card'><h4>AI-ассистент</h4><div className='ui-card'><b>Рекомендации</b><p className='sub'>Добавьте BPMN диаграмму AS IS для более точного анализа.</p></div><div className='ui-card'><b>Следующий шаг</b><p className='sub'>Перейдите к разделу «Проблема» для анализа.</p><button className='ui-btn secondary'>Перейти к проблеме</button></div><div className='ui-card'><b>История индексации</b><p className='sub'>Успешно: документы и ссылки проиндексированы.</p></div><div style={{marginTop:'auto'}}><input className='ui-input' placeholder='Спросите что угодно...'/></div></div>
        </div> : active==='PROBLEM' ? <div className='problem-workspace'>
          <div className='problem-left ui-card'><h4>Контекст для анализа</h4><div className='sub'>{contextInput.initiative_name}</div><div className='sub'>{contextInput.short_description}</div><div className='sub'>Процессы: {safeText((knowledge?.процессы||knowledge?.processes||[]).slice(0,5))}</div><div className='sub'>Системы: {safeText((knowledge?.системы||knowledge?.systems||[]).slice(0,5))}</div><div className='sub'>Роли: {safeText((knowledge?.роли||knowledge?.roles||[]).slice(0,5))}</div><button className='ui-btn secondary' onClick={()=>loadArtifact('CONTEXT')}>Обновить из контекста</button>{problemDraft.source_context_version && ver && problemDraft.source_context_version<ver ? <div className='ui-card'><p className='sub'>Контекст изменился. Рекомендуется обновить анализ проблемы.</p><button className='ui-btn secondary' onClick={generateProblem}>Обновить проблему по новому контексту</button></div>:null}</div>
          <div className='problem-center ui-card'><h4>Формулировка проблемы</h4><label className='sub'>Ручная формулировка от PO</label><textarea className='ui-textarea' placeholder='В чём проблема?' value={problemDraft.main_problem||''} onChange={e=>setProblemDraft({...problemDraft,main_problem:e.target.value})}/><label className='sub'>Проблемы для БТ (AI, строгая нумерация)</label><textarea className='ui-textarea' value={problemDraft.problem_statement||''} onChange={e=>setProblemDraft({...problemDraft,problem_statement:e.target.value})}/><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button className='ui-btn primary' onClick={generateProblem} disabled={thinking}>{thinking?'Генерация...':'Сгенерировать'}</button><button className='ui-btn secondary' onClick={()=>saveProblem()}>Сохранить</button><button className='ui-btn secondary' onClick={()=>saveProblem('ready')}>Принять как финальную проблему</button><button className='ui-btn secondary' onClick={()=>{const v=(problemDraft.versions||[]).slice(-1)[0]?.snapshot; if(v) setProblemDraft({...problemDraft,...v})}}>Вернуть предыдущую версию</button></div><div className='sub'>Статус: {safeText(problemDraft.status||'draft')}</div></div>
          <div className='problem-right ui-card'><h4>AI уточняющие вопросы</h4><button className='ui-btn secondary' onClick={regenerateStageQuestions} disabled={questionGenLoading}>{questionGenLoading?'Генерация...':'Сгенерировать дополнительные вопросы'}</button><div className='ai-sections'>{(problemDraft.clarifying_questions||[]).map((q:any,i:number)=>{const obj=typeof q==='string'?{id:`q_${i}`,text:q,answer:''}:q; return <div key={obj.id||i} className='ui-card'><b>Вопрос {i+1}</b><div className='sub'>{safeText(obj.text||obj)}</div><input className='ui-input' placeholder='Ваш ответ...' value={questionAnswers[obj.id] ?? obj.answer ?? ''} onChange={e=>setQuestionAnswers({...questionAnswers,[obj.id]:e.target.value})}/><button className='ui-btn secondary' onClick={()=>answerQuestion(obj)} disabled={answerLoadingId===obj.id}>{answerLoadingId===obj.id?'Сохранение...':'Сохранить ответ'}</button></div>})}{(problemDraft.ai_chat_history||[]).map((m:any,i:number)=><div key={i} className='ui-card'><b>{m.role==='user'?'Вы':'AI'}</b><div className='sub'>{safeText(m.text)}</div></div>)}</div><div className='ui-card'><h4 style={{marginBottom:8}}>Ответы пользователя</h4>{(problemDraft.clarifying_questions||[]).map((q:any,i:number)=>{const obj=typeof q==='string'?{id:`q_${i}`,text:q}:q; const answer=(questionAnswers[obj.id] ?? obj.answer ?? '').trim(); if(!answer) return null; return <div key={`qa_${obj.id||i}`} style={{marginBottom:8}}><div><b>В:</b> {safeText(obj.text||obj)}</div><div className='sub'><b>О:</b> {answer}</div></div>})}<div className='sub'>Здесь фиксируются только вопросы с заполненными ответами.</div></div>{stagePatch && <div className='ui-card'><div className='sub'>AI предлагает изменения.</div><div style={{display:'flex',gap:8}}><button className='ui-btn primary' onClick={applyStagePatch}>Применить</button><button className='ui-btn secondary' onClick={()=>setStagePatch(null)}>Отклонить</button></div></div>}</div>
          ...
          <div className='ui-card'><h4>Целевая формулировка</h4>
            ...
            <textarea className='ui-textarea' style={{minHeight:80}} placeholder='Какую проблему решаем' value={goalData.businessProblem||''} onChange={e=>updateGoal({businessProblem:e.target.value})}/>
            <textarea className='ui-textarea' style={{minHeight:80}} placeholder='Какой результат хотим получить' value={goalData.desiredOutcome||''} onChange={e=>updateGoal({desiredOutcome:e.target.value})}/>
            <textarea className='ui-textarea' style={{minHeight:70}} placeholder='Почему это важно бизнесу' value={goalData.businessImportance||''} onChange={e=>updateGoal({businessImportance:e.target.value})}/>
            <textarea className='ui-textarea' style={{minHeight:70}} placeholder='Что произойдет, если ничего не делать' value={goalData.noActionImpact||''} onChange={e=>updateGoal({noActionImpact:e.target.value})}/>
            <textarea className='ui-textarea' style={{minHeight:70}} placeholder='Что НЕ входит в scope' value={(goalData.nonGoals||[]).join('\n')} onChange={e=>updateGoal({nonGoals:e.target.value.split('\n').filter(Boolean)})}/>
            <textarea className='ui-textarea' style={{minHeight:70}} placeholder='Ограничения' value={(goalData.constraints||[]).join('\n')} onChange={e=>updateGoal({constraints:e.target.value.split('\n').filter(Boolean)})}/>
            <textarea className='ui-textarea' style={{minHeight:70}} placeholder='Предпосылки и допущения' value={(goalData.assumptions||[]).join('\n')} onChange={e=>updateGoal({assumptions:e.target.value.split('\n').filter(Boolean)})}/>
          </div>
          <div className='ui-card'><h4>Метрики успеха</h4><button className='ui-btn secondary' onClick={addMetric}>Добавить метрику</button>{(goalData.successMetrics||[]).map((m:any,i:number)=><div key={i} className='metric-row'><input className='ui-input' placeholder='Метрика' value={m.metric||''} onChange={e=>{const arr=[...(goalData.successMetrics||[])]; arr[i]={...m,metric:e.target.value}; updateGoal({successMetrics:arr})}}/><input className='ui-input' placeholder='Текущее значение' value={m.currentValue||''} onChange={e=>{const arr=[...(goalData.successMetrics||[])]; arr[i]={...m,currentValue:e.target.value}; updateGoal({successMetrics:arr})}}/><input className='ui-input' placeholder='Целевое значение' value={m.targetValue||''} onChange={e=>{const arr=[...(goalData.successMetrics||[])]; arr[i]={...m,targetValue:e.target.value}; updateGoal({successMetrics:arr})}}/><input className='ui-input' placeholder='Как измеряем' value={m.measurement||''} onChange={e=>{const arr=[...(goalData.successMetrics||[])]; arr[i]={...m,measurement:e.target.value}; updateGoal({successMetrics:arr})}}/><input className='ui-input' placeholder='Источник данных' value={m.dataSource||''} onChange={e=>{const arr=[...(goalData.successMetrics||[])]; arr[i]={...m,dataSource:e.target.value}; updateGoal({successMetrics:arr})}}/></div>)}</div>
          <div className='ui-card'><h4>SMART-анализ</h4>{Object.entries(goalData.smartAnalysis||{}).map(([k,v]:any)=><div key={k} className='smart-row'><div className='sub'><b>{k}</b>: {v.comment}</div><button className='ui-btn secondary'>Исправить с AI</button></div>)}</div>
          <div className='ui-card'><h4>AI Drafts</h4>{goalDraft ? (goalDraft.goal_options||[]).map((o:any,i:number)=><div key={i} className='ui-card'><b>{o.title||`Вариант ${i+1}`}</b><div className='sub'>{o.description}</div><div style={{display:'flex',gap:8}}><button className='ui-btn primary' onClick={()=>applyGoalOption(o,'select')}>Выбрать</button><button className='ui-btn secondary' onClick={()=>applyGoalOption(o,'merge')}>Объединить</button><button className='ui-btn secondary'>Редактировать</button></div></div>) : <div className='sub'>Сначала запустите генерацию целей.</div>}</div>
          ...
          <div className='ui-card'><h4>AI Questions</h4>{(goalData.aiQuestions||[]).map((q:string,i:number)=><div key={i} className='sub'>• {q}</div>)}<input className='ui-input' placeholder='Ответьте на вопрос AI...' value={goalQuestion} onChange={e=>setGoalQuestion(e.target.value)}/><button className='ui-btn secondary' onClick={askGoal}>Отправить AI</button>{goalPatch && <div className='ui-card'><div className='sub'>AI предлагает patch</div><div style={{display:'flex',gap:8}}><button className='ui-btn primary' onClick={applyGoalPatch}>Применить</button><button className='ui-btn secondary' onClick={()=>setGoalPatch(null)}>Отклонить</button></div></div>}</div>
        ...
        {active==='CONTEXT' && <div className='ui-card' style={{marginTop:12}}><h4 style={{marginTop:0}}>Как работает Discovery в платформе</h4><div className='flow-grid'>{workflowStages.map(([key,text],idx)=><div key={key} className={`flow-step ${active===key?'active':''}`}><b>{idx+1}. {tabs.find(t=>t.type===key)?.label}</b><p className='sub'>{text}</p></div>)}</div><p className='sub'>AI постоянно использует контекст на всех этапах и обновляет артефакты при изменениях.</p></div>}
        {active==='FINAL_BT' && <div className='ui-card' style={{marginTop:12}}><h4 style={{marginTop:0}}>Preview финального БТ</h4><div className='sub'>Sections, зависящие от Goal: BUSINESS_EFFECT, AS_IS, TO_BE, USE_CASES, REQUIREMENTS, RISKS, FINAL_BT.</div></div>}<div className='sub'>Autosave: {saving==='saving'?'Сохранение…':saving==='saved'?'Сохранено':saving==='error'?'Ошибка сохранения':'—'}</div>
