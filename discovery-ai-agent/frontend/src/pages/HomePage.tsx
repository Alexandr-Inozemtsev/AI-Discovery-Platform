...
export default function HomePage() {
  ...
  return <PageContainer>
    ...
    <div className='gridCards' style={{ marginTop: 14 }}>
      ...
      <Card><h3>Быстрые действия</h3><div style={{ display: 'grid', gap: 8 }}><Link className='ui-btn secondary' to='/projects'><PlusCircle size={16} /> Создать проект</Link><Button><FileUp size={16} /> Импортировать БТ</Button><Button onClick={() => last && navigate(`/projects/${last.id}`)}><Clock3 size={16} /> Продолжить последний проект</Button><Button><CheckCheck size={16} /> Проверить полноту</Button><Button><FileDown size={16} /> Экспортировать БТ</Button></div></Card>
